# Sprint 3 (Week 3: Days 14–18) — In-House LMS Build (Part 2) & Scopes

**Function Head**: Gaurav (CTO) | **AI PM Lead**: Yashvi  
**Sprint Window**: Monday, Sept 14 (Day 14) – Friday, Sept 18 (Day 18), 2026 *(Extending to Day 20/21 milestones on Sept 20/21)*  
**Daily Standup**: **8:00 PM IST** (Async notes by 7:30 PM) | **Weekly Review**: **Friday, Sept 18 at 8:00 PM IST**  
**Sprint Status**: `[ ] Planned` | **RAG**: 🟢 ON TRACK  

---

## 🎯 Sprint 3 Core Goals (High-Level & Quantifiable)

* **C1: Market & Intake**: **Publish Catalogues v1 (Mon, Sept 14 / Day 14)** + Secure **3 Accepted Written POC Scopes (Sun, Sept 20 / Day 20)**.
* **C2: Product & Demo**: **In-House LMS Prototype Build (Part 2: Student OS & Test Integration into Mr. Learn)** + **Demo #3: Sales & Admin Suite (Prakhar)** + start QA regression. *(Gap Sheet deferred to Month 2)*.
* **C3: Cloud & Cost**: Switch on **AI Cost Instrumentation Baseline (Mon, Sept 21 / Day 21 Milestone)** + separate dev/demo/prod + build 1st automation prototype.
* **C4: Leadership & Enablers**: **Onboard QA Tester & PM Intern (Mon, Sept 21 / Day 21 Milestone)** + advance GPU vendor terms with Sajan.

---

## 📦 Compartment 1: Market, Intake & Client POCs

| ID | Task / Activity | Owner (Support) | Target Date (Day) | Status | Expected Outcome | Actual Outcome | Blocker | Delay Reason | RAG |
|:---:|---|:---:|:---:|:---:|---|---|---|---|:---:|
| **S3.1** | **Publish Product Catalogue v1 (M5)** | **Prakhar** *(Gaurav)* | **Mon, Sept 14 (Day 14)** | `[ ] Planned` | **Catalogue v1 complete & published for Business** | - | None | None | ⚪ |
| **S3.2** | **Publish Capability Registry v1 (M5)** | **Yashvi** *(Shubham)* | **Mon, Sept 14 (Day 14)** | `[ ] Planned` | **Registry v1 seeded & published for Business** | - | None | None | ⚪ |
| **S3.3** | Conduct technical feasibility review on 3 client scopes | **Shubham** *(Yashvi)* | Wed, Sept 16 (Day 16) | `[ ] Planned` | Feasibility note & rough effort estimate per client | - | None | None | ⚪ |
| **S3.4** | Finalize written POC Scope: **Orane** | **Yashvi** *(Prakhar)* | Thu, Sept 17 (Day 17) | `[ ] Planned` | Written scope document ready for Business & client sign-off | - | None | None | ⚪ |
| **S3.5** | Finalize written POC Scope: **College Vidya** | **Yashvi** *(Prakhar)* | Thu, Sept 17 (Day 17) | `[ ] Planned` | Written scope document ready for Business & client sign-off | - | None | None | ⚪ |
| **S3.6** | Finalize written POC Scope: **Chitkara** | **Yashvi** *(Prakhar)* | Fri, Sept 18 (Day 18) | `[ ] Planned` | Written scope document ready for Business & client sign-off | - | None | None | ⚪ |
| **S3.7** | **Secure 3 Accepted Written POC Scopes (M1)** | **Yashvi** *(Prakhar)* | **Sun, Sept 20 (Day 20)** | `[ ] Planned` | **3 Written POC scopes accepted by client and Business** | - | None | None | ⚪ |
| **S3.8** | Mandatory intake process enforcement on all new leads | **Prakhar** *(Business)* | Daily | `[ ] Planned` | 100% of new client requests enter through standard intake | - | None | None | ⚪ |

---

## 📦 Compartment 2: Product, In-House LMS & Demo Stabilisation

| ID | Task / Activity | Owner (Support) | Target Date (Day) | Status | Expected Outcome | Actual Outcome | Blocker | Delay Reason | RAG |
|:---:|---|:---:|:---:|:---:|---|---|---|---|:---:|
| **S3.9** | **In-House LMS Build (Part 2: Student OS & Progress Tracking)** | **Shubham** *(Rohan)* | Wed, Sept 16 (Day 16) | `[ ] Planned` | Student profile, module progress & analytics tracking module | - | None | None | ⚪ |
| **S3.10** | **In-House LMS Integration with Mr. Learn & Mr. Test** | **Shubham** *(Rohan)* | Fri, Sept 18 (Day 18) | `[ ] Planned` | Working prototype connecting Mr. Learn UI to in-house LMS | - | None | None | ⚪ |
| **S3.11** | Create short 15-min demo flow/script per suite | **Shubham** *(Prakhar, Yashvi)* | Wed, Sept 16 (Day 16) | `[ ] Planned` | Business-ready demo flow & narrative scripts | - | None | None | ⚪ |
| **S3.12** | **Demo #3: Sales & Admin Suite Walkthrough** | **Prakhar** *(Shubham)* | **Thu, Sept 17 (Day 17)** | `[ ] Planned` | **Weekday demo of Sales & Admin Suite** | - | None | None | ⚪ |
| **S3.13** | Start QA regression test suite on P0 products | **QA / Tester** *(Shubham)* | Fri, Sept 18 (Day 18) | `[ ] Planned` | Regression test cases drafted and executing | - | None | None | ⚪ |

*(Note: Externalisation Gap Sheet is explicitly scheduled for Month 2 engineering backlog)*.

---

## 📦 Compartment 3: Cloud, Cost & Internal Automation

| ID | Task / Activity | Owner (Support) | Target Date (Day) | Status | Expected Outcome | Actual Outcome | Blocker | Delay Reason | RAG |
|:---:|---|:---:|:---:|:---:|---|---|---|---|:---:|
| **S3.14** | Restructure cloud projects & accounts where required | **Rohan** *(Shubham)* | Tue, Sept 15 (Day 15) | `[ ] Planned` | Clean logical separation across GCP/AWS | - | None | None | ⚪ |
| **S3.15** | Separate dev / demo / production environments | **Rohan** *(Shubham)* | Thu, Sept 17 (Day 17) | `[ ] Planned` | Isolated demo environment preventing test contamination | - | None | None | ⚪ |
| **S3.16** | Select top 1 simple, high-value departmental workflow | **Rohan** *(Gaurav)* | Wed, Sept 16 (Day 16) | `[ ] Planned` | Target automation selected for Month 1 build | - | None | None | ⚪ |
| **S3.17** | Build & test first internal automation workflow prototype | **Rohan** | Fri, Sept 18 (Day 18) | `[ ] Planned` | Core automation script/integration built | - | None | None | ⚪ |
| **S3.18** | **Switch on AI Cost Instrumentation Baseline (M4)** | **Rohan** *(Shubham)* | **Mon, Sept 21 (Day 21)** | `[ ] Planned` | **Product-level token/API spend visible for live workloads** | - | None | None | ⚪ |

---

## 📦 Compartment 4: Leadership, Compute & Enablers

| ID | Task / Activity | Owner (Support) | Target Date (Day) | Status | Expected Outcome | Actual Outcome | Blocker | Delay Reason | RAG |
|:---:|---|:---:|:---:|:---:|---|---|---|---|:---:|
| **S3.19** | Advance GPU compute/vendor discussions with Sajan | **Gaurav** *(Sajan)* | Thu, Sept 17 (Day 17) | `[ ] Planned` | Hardware spec options, vendor terms, and cost projection | - | None | None | ⚪ |
| **S3.20** | **Weekly Product & Capability Review #3 (8:00 PM)** | **Gaurav** *(All)* | Fri, Sept 18 (Day 18) | `[ ] Planned` | Review minutes, LMS Prototype Part 2 preview & ad-hoc audit | - | None | None | ⚪ |
| **S3.21** | **Onboard QA Tester & PM Intern (M8)** | **Gaurav** *(HR/Admin)* | **Mon, Sept 21 (Day 21)** | `[ ] Planned` | **QA active on P0 regression; Intern active on tasks** | - | None | None | ⚪ |

---

## 📊 Workload & Ad-Hoc Effort Tracking (Sprint 3)
* **Primary Work Consuming Time (Time Sinks)**: `[Log key focus areas / deep work items]`
* **Incoming Ad-Hoc MAS Requests**: `[Log any incoming ad-hoc or custom requests]`
* **Estimated Ad-Hoc Workload Share**: `0%` *(Guardrail: Must stay ≤ 20–30% of total team effort)*

---

## 📝 Daily Quick Updates Log

---
*(Bot automatically appends the single final daily summary after each evening's standup)* 
* *Mon, Sept 21 (Day 21 Milestone Check)*: 


